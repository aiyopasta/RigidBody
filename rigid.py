...
def valid_plane(pair: frozenset, r0: np.ndarray, nor: np.ndarray, get_degenerates=False):
    '''
        ...
        2. We'll say a separating plane satisfies the following:
            — ALL the verts of one of the bodies must result in f(v) > 0  (strict positivity), AND
            — ALL the verts of the other must result in f(v) <= 0  (weak negativity). Cuz plane will always contain
              one edge of one of the bodies.


        get_degenerates: Boolean, if True then if the plane is not valid, we get a list of vertices that
    ...
    first_was_strict_positive = np.dot(nor, pts[0] - r0) > eps
    # 1. If this random "first" vertex of the first body is strictly positive, check if ALL others are too.
    #    Keep track of those that aren't, these are degens.
    if first_was_strict_positive:
        for vert in pts:
            is_strict_positive = np.dot(nor, vert - r0) > eps
            if not is_strict_positive:
                degens.append(vert)
        # If you found any degens, we're done! This is only possible if the separating plane is invalid, since
        # the shape hugging the separating plane MUST be weakly negative for ALL it's verts, but this one has
        # at least one strongly positive one, and at least one not strongly positive one.
        if len(degens) != 0:
            return degens if get_degenerates else False
        # Otherwise, we're still done! Since we found no degens, it means ALL the sides are strictly positive,
        # meaning there's no way both shapes could be on the same side, since again, one of the shapes (namely
        # the one hugging the plane) must be weakly negative.
        else:
            return degens if get_degenerates else True

    # 2. If that random first vertex was NOT strictly positive (i.e. it's weakly negative), then check if all
    #    others are too. Keep track of: 1) All the weakly negative ones you find (they could be degens), and
    #    2) Whether you see any strictly positive ones.
    else:
        found_strict_positive = False
        for vert in pts:
            is_weakly_neg = np.dot(nor, vert - r0) <= eps
            if is_weakly_neg:
                degens.append(vert)  # this'll include the first vertex too
            else:
                found_strict_positive = True
        # If we did find a strictly positive one, the list of weakly negative ones were indeed degens! So let's
        # return them.
        if found_strict_positive:
            return degens if get_degenerates else False
        # Otherwise, it means we've simply found the body hugging the separating plane. The list of verts are not
        # degens after all, so clear them. But now we need to check the verts of the second body and make sure
        # ALL its verts are strictly positive. We'll keep track of those that aren't, as these are degens.
        degens.clear()
        raw_pts = b2.get_poly_points(screen_space=False)
        pts = np.reshape(raw_pts, (int(len(raw_pts) / 2), 2))
        for vert in pts:
            is_strictly_positive = np.dot(nor, vert - r0) > eps
            if not is_strictly_positive:
                degens.append(vert)
        # If we still did not find any degens, that means the plane is valid! Indeed, one of the shapes has all
        # its vertices as weakly negative and the other shape has all its verts as strictly positive. Otherwise,
        ...
        if mode == 0:
            for body in bodies:
                ...
                if 1 <= body.solver_id <= 2:
                    ...
                    color = 'green'
                w.create_text(*(A(body.xy) % [window_w, window_h] - np.array([0, 60])), text=str(body.solver_id),
                              fill=color, font='Avenir 25')
                w.create_polygon(*body.get_poly_points(), fill=color, outline='white')
                w.create_oval(body.get_centroid_points(), fill='blue')

            if gravity:
                if frame < max_frame:
                    w.create_text(window_w / 2, window_h / 2,
                                  text='Competition Mode: Frame #' + str(frame) + '/' + str(max_frame),
                                  font='Avenir 50', fill='white')
                elif frame < max_frame + 100:
                    winner_text = '1st Place — S.I. Methods (Greens)!'
                    w.create_text(window_w / 2, window_h / 2, text=winner_text, font='Avenir 50', fill='white')
            else:
                w.create_text(window_w / 2, window_h / 2,
                              text='Space mode (constant velocity, so all methods match exactly!)', font='Avenir 50',
                              fill='white')
            ...
            for i, body in enumerate(bodies):
                ...
                w.create_line(*(topleft + np.array([0, energy * scaling])),
...
def mouse_click(event):
    ...
    if mode == 0 and ((len(bodies) == 0 and gravity) or (not gravity)):
        ...
        spacing = np.array([20.0 + (sidelen / np.sin(np.pi / rng_sides)),
                            0])  # from centroid to centroid. formula is for diameter of a /regular/ poly (good approx) plus some offset
        for i in range(5):
            xy = clicked_pt + (-2 * spacing) + (i * spacing)
            body = RigidBody(verts, init_xy=copy.copy(xy), init_v=copy.copy(rng_v), init_theta=copy.copy(rng_theta),
                             init_omega=copy.copy(rng_omega), solver_id=i)
            bodies.append(body)
...
#     # 2) Exact solution for 2nd or 1st order ODE (linear or quadratic), using Taylor expansion at CURRENT TIME STEP.
#     #    This is also the TRAPEZOID RULE (2nd order) Runge Kutta method! (The one we used in 562 as well.)
#     #    In other literature, this is also known as the Improved Euler's method or Heun's method.
#     def solve_exact_tk(self, h, const_a=np.array([0,0]), const_alpha=0.):
#         # Exact solution from truncated Taylor expansion of x at t_{k+1} = t_k + h, namely:
#         # x(t_{k+1}) = x(t_k) + h*xdot(t_k) + h^2/2! * xddot(t_k).
#         self.xy += (h * self.v) + (np.power(h, 2) * const_a / 2.0)
...
#
#     # 3) Exact solution for 2nd or 1st order ODE (linear or quadratic), using Taylor expansion at INITIAL TIME STEP.
#     #    In some literature, these basic kinematic equations (i.e. x0 + v0t + 1/2at^2) are referred to as the SUVAT
#     #    equations, which is a stupid acronym (not an initialism!) for something like "distance, acceleration,
#     #    time", blah blah.
#     def solve_exact_t0(self, h, const_a=np.array([0,0]), const_alpha=0.):
#         self.n += 1
#         elapsed_t = h * self.n
...
#
# # Mode 2 Parameters
# restitution_coeff = 0.5  # will not work for too low because haven't implemented resting contact!
# class Endpoint:
#     kinds = ['X-Start', 'X-End', 'Y-Start', 'Y-End']
#
#     def __init__(self, body_idx, value, kind=0):
...
# # Dict from a 2-FROZENSET representing a collision pair, to a 2-tuple / 2-list representing the plane.
# # The plane is represented as (r0, w, i) where r0 and v are np.arrays representing the offset and normal, respectively,
# # IN THE SHAPE'S LOCAL SPACE! Otherwise, it's no use! For every rotational / positional change we'd have to recompute,
# # and i is the index of the body whose local frame is the one we're using to describe r0 and w.
# # i.e. The test is then whether f(x) = w • (x - r0) is positive or negative.
# # NOTE: We'll NEVER remove pairs from this dict. It doesn't matter. If a pair is not a valid bbox collision, we just
# #       won't access it, and some dirty data will remain its value, which'll be cleaned automatically by checking
...
#         # End run
#         w.update()
#         # TODO: It seems to be detecting the WRONG collision points sometimes... like it's highlighting the 3 points
#         #       out of the one that's actually relevant sometimes... Also, when you set restitution coefficient to 0.5
#         #       for example the objects easily tunnel through each-other even if you eliminate "resting contact" as a
#         #       category... need to debug.
#         print('Frame',count)
#         if count > 220:
#             time.sleep(dt * 20)
#         else:
